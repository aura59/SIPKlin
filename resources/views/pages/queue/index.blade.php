...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <form action="{{ route('queues.index') }}" method="GET" class="queue-filter">
            ...
            <button type="submit" class="btn btn-sipklin btn-tampilkan">
                Tampilkan
            </button>
            ...
            <div class="card queue-info-card h-100">
                ...
                <div class="card-body">
                    <div class="poli-information text-center">
                        ...
                        <h5>{{ $selectedDepartment->name ?? 'Pilih Poli' }}</h5>
                    ...
                    <div class="current-queue-box">
                        ...
                        @if ($currentRegistration)
                            ...
                            @php
                                $currentNumber = $registrations
                                    ->search(function ($item) use ($currentRegistration) {
                                        return $item->id == $currentRegistration->id;
                                    }) + 1;

                                $currentCode = $currentRegistration->doctorSchedule->doctor->department->kode_poli ?? 'A';
                            @endphp

                            <strong>{{ $currentCode }}-{{ str_pad($currentNumber, 2, '0', STR_PAD_LEFT) }}</strong>
                    ...
                    <div class="next-queue-info">
                        ...
                        @if ($nextRegistration)
                            ...
                            @php
                                $nextNumber = $registrations
                                    ->search(function ($item) use ($nextRegistration) {
                                        return $item->id == $nextRegistration->id;
                                    }) + 1;

                                $nextCode = $nextRegistration->doctorSchedule->doctor->department->kode_poli ?? 'A';
                            @endphp

                            <strong>{{ $nextCode }}-{{ str_pad($nextNumber, 2, '0', STR_PAD_LEFT) }}</strong>
                            ...
                            <tbody>
                                ...
                                @forelse ($registrations as $registration)
                                    ...
                                    <tr>
                                        ...
                                        <td><strong>{{ $kodePoli }}-{{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}</strong></td>
                                        <td>{{ $registration->patient->nama ?? '-' }}</td>
                                        <td>{{ $registration->created_at->format('H:i') }}</td>
                                        <td>
